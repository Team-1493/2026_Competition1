...
class ShootCommandAutoFixed(commands2.Command):
    def __init__(self, shoot_speed=-1.0, shoot_time=4.0):
...
#        self.intake.arm_up()
        ...
        SmartDashboard.putNumber("Shooter Speed Calculated",self.shoot_speed)        
        self.shooter.shoot(self.shoot_speed)
...
#            self.intake.arm_to_position(0.1 + 0.1*math.sin(1.5*self.timer.get()*math.pi) )
    ...
    @override
    def isFinished(self):
        return self.timer.get()>self.shoot_time
