...
class HeadingController(Subsystem):
    ...
    def rotateTo90(self):
        if self.is_near_wall():
            self.start_snap_away_from_wall(pi/2)
        else:
    ...
    def rotateTo270(self):
        if self.is_near_wall():
            self.start_snap_away_from_wall(3*pi/2)
        else:
    ...
    def get_snap_rotation_rate(self, max_angular_rate: float) -> float:
        if self.state != 3 or self.snap_target_unwrapped is None:
            return 0.0
    ...
    def start_snap_away_from_wall(self, target_angle: float):
        ...
        if self.is_snap_toward_wall_risky(short_delta_to_target):
        ...
        self.snap_rotation_direction = 1 if delta_to_target > 0 else -1
        self.state = 3
